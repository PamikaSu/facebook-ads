with ads as (
  ...
  select * from {{ref('fb_adsets_xf')}}
    ...
    left outer join ads
        ...
        and insights.date_day >= cast({{ dbt_utils.date_trunc('day', 'ads.effective_from') }} as date)
        and (insights.date_day < cast({{ dbt_utils.date_trunc('day', 'ads.effective_to') }} as date) or ads.effective_to is null)
